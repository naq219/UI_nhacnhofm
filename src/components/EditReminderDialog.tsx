...
interface EditReminderDialogProps {
  ...
  reminder: Reminder;
  onEditReminder: (reminder: Partial<Reminder>) => void;
}
...
export function EditReminderDialog({
  ...
  const [date, setDate] = useState<Date>(new Date(reminder.startTime));
  const [time, setTime] = useState('');
  const [calendarType, setCalendarType] = useState<'solar' | 'lunar'>(reminder.calendarType);
  ...
  const [isImportant, setIsImportant] = useState(reminder.isImportant);
  const [enableRetry, setEnableRetry] = useState(reminder.enableRetry);
  const [retryIntervalSec, setRetryIntervalSec] = useState(reminder.retryIntervalSec || 300);
  const [maxRetries, setMaxRetries] = useState(reminder.maxRetries || 3);
  ...
  useEffect(() => {
    ...
    setCalendarType(reminder.calendar_type);
    setIsRecurring(reminder.type === 'recurring');
    setIsImportant(false); // Không có trường isImportant trong kiểu mới
    setEnableRetry(false); // Không có trường enableRetry trong kiểu mới
    setRetryIntervalSec(reminder.retry_interval_sec || 300);
    setMaxRetries(reminder.max_retries || 3);

    // Determine repeat type from reminder - kiểu mới không hỗ trợ repeatUnit/repeatValue
    // Tạm thời đặt mặc định là daily cho recurring reminders
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    let repeatInterval = '';
    let repeatUnit: 'minute' | 'hour' | 'day' | 'week' | 'month' | 'year' | undefined;
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-4">
            <div className="flex items-center gap-2 text-blue-700">
              ...
              <span className="text-sm">Chọn nhanh</span>
            </div>
          ...
          <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-slate-700">
                ...
                <span className="text-sm">Thời gian lúc</span>
              </div>
              <RadioGroup value={calendarType} onValueChange={(v: any) => setCalendarType(v)} className="flex gap-2">
                <div className="flex items-center space-x-1.5">
                  <RadioGroupItem value="solar" id="edit-solar" className="h-3.5 w-3.5" />
                  <Label htmlFor="edit-solar" className="flex cursor-pointer items-center gap-1 text-xs">
                    <Sun className="h-3.5 w-3.5 text-amber-500" />
                    Dương lịch
                  </Label>
                </div>
                <div className="flex items-center space-x-1.5">
                  <RadioGroupItem value="lunar" id="edit-lunar" className="h-3.5 w-3.5" />
                  <Label htmlFor="edit-lunar" className="flex cursor-pointer items-center gap-1 text-xs">
                ...
                <RadioGroup value={repeatType} onValueChange={(v: any) => setRepeatType(v)}>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <Label htmlFor="edit-daily" className="cursor-pointer">
                        Mỗi ngày
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="weekly" id="edit-weekly" />
                      <Label htmlFor="edit-weekly" className="cursor-pointer">
                        Mỗi tuần
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="monthly" id="edit-monthly" />
                      <Label htmlFor="edit-monthly" className="cursor-pointer">
                        Mỗi tháng
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
            ...
            {enableRetry && (
              <div className="space-y-3 pt-2">
                <RadioGroup value={retryType} onValueChange={(v: any) => setRetryType(v)}>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="daily" id="edit-retry-daily" />
                      <Label htmlFor="edit-retry-daily" className="cursor-pointer text-xs">
                        Mỗi ngày
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="weekly" id="edit-retry-weekly" />
                      <Label htmlFor="edit-retry-weekly" className="cursor-pointer text-xs">
                        Mỗi tuần
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="monthly" id="edit-retry-monthly" />
                      <Label htmlFor="edit-retry-monthly" className="cursor-pointer text-xs">
                        Mỗi tháng
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="custom" id="edit-retry-custom" />
                      <Label htmlFor="edit-retry-custom" className="cursor-pointer text-xs">
                        Tùy chọn
                      </Label>
                    </div>
                  </div>
                ...
                {retryType !== 'custom' && (
                  <div className="space-y-2">
                    <Label htmlFor="edit-maxRetries" className="text-xs text-slate-600">
                      Số lần tối đa
                    </Label>
                    <Input
                      id="edit-maxRetries"
